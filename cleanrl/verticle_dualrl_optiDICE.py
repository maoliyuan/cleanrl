...
"""
    ...
    af = AdvantageNetwork(envs).to(device)
    ...
    advantage_optimizer = optim.Adam(list(af.parameters()), lr=args.learning_rate)
        ...
        if global_step > args.learning_starts:
            for substep in range(args.sample_efficiency):
                ...
                if (global_step * args.sample_efficiency + substep + 1) % args.value_frequency == 0:
                    ...
                    adv_pred = af(data.observations, data.actions).view(-1)
                    adv_loss= F.mse_loss(adv_pred, pi_residual)
                    advantage_optimizer.zero_grad(set_to_none=True)
                    adv_loss.backward()
                ...
                if (global_step * args.sample_efficiency + substep + 1) % args.policy_frequency == 0:
                    action_pi, log_action_pi = actor.get_action(data.observations)
                    policy_loss = -1 * torch.mean(af(data.observations, action_pi) - args.beta * log_action_pi)
